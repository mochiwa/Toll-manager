...
BEGIN
	SELECT * INTO v_employee FROM employee WHERE employee_id=v_idEmployee;
	RETURN v_employee;
END $$ LANGUAGE plpgsql;
...
DECLARE
	v_employee employee%ROWTYPE;
BEGIN
	SELECT * INTO v_employee FROM employee WHERE employee_niss=v_niss;
	RETURN v_employee;
END $$ LANGUAGE plpgsql;
